#####
# Script R para analise de desempenho de memória com uso exclusivo
...
tabelaDados <- read.table('../Tabelas/TabelaMemLeituraEx.log',
                          header = T)
...
memoriaLeituraEx <- ggplot(tabelaDados, aes(fill = Ambiente, x = GB, y = Tempo_Medio, group = Ambiente)) +
...
######
## Os mesmos gráficos foram gerados (media e coeficiente de variação)
## mas agora sem os 3 maiores outliers do ambiente kvm_1
######
tabelaDadosOutliers <- read.table('../Tabelas/TabelaMemLeituraExOutliers.log',
                          header = T)

tabelaDadosOutliers <- tabelaDadosOutliers %>%
  group_by(GB, Ambiente) %>%
  summarize(Tempo_Medio = mean(Tempo),
            Tempo_Minimo = min(Tempo),
            Tempo_Maximo = max(Tempo),
...
tabelaDadosOutliers$GB <- factor(tabelaDadosOutliers$GB, levels = c(unique(tabelaDadosOutliers$GB)[order(unique(tabelaDadosOutliers$GB))]))
### Gráfico em barras com o tempo médio de desempenho de cada ambiente + intervalo de confiança
memoriaLeituraExOutliers <- ggplot(tabelaDadosOutliers, aes(fill = Ambiente, x = GB, y = Tempo_Medio, group = Ambiente)) +
  geom_bar(position = 'dodge', stat = 'identity') +
  geom_errorbar(aes(ymin=Tempo_Medio - Intervalo_Confianca, ymax=Tempo_Medio+Intervalo_Confianca),
                position='dodge') +
  geom_errorbar(aes(ymin=Tempo_Medio - Intervalo_Confianca, ymax=Tempo_Medio+Intervalo_Confianca),
...
### Gráfico com Coeficiente de Variação dos ambientes 
### Esse gráfico tem o objetivo de apresentar a variação dos dados obtidos em porcentagem
coeficienteLeituraExOutliers <- ggplot(tabelaDadosOutliers, aes(fill = Ambiente, x = GB, y = Coeficiente_Variacao, group = Ambiente)) +
  geom_bar(position = 'dodge', stat = 'identity') +
  labs(title="Coefiente de Variação dos Resultados",x="GB", y = "Coeficiente_Variação (%)")+
  theme_bw(base_size = 20) +
  scale_fill_manual(values=c("#756bb1", "#fdae6b", "#31a354")) +
...
#######
### Análise de Escrita Exclusiva sem os outliers do KVM_1
###
#######
tabelaEscritaExOutliers <- read.table('../Tabelas/TabelaMemEscritaExOutliers.log',
                              header = T)

tabelaEscritaExOutliers <- tabelaEscritaExOutliers %>%
  group_by(GB, Ambiente) %>%
  summarize(Tempo_Medio = mean(Tempo),
            Tempo_Minimo = min(Tempo),
            Tempo_Maximo = max(Tempo),
...
tabelaEscritaExOutliers$GB <- factor(tabelaEscritaExOutliers$GB, levels = c(unique(tabelaEscritaExOutliers$GB)[order(unique(tabelaEscritaExOutliers$GB))]))
### Gráfico em barras com o tempo médio de desempenho de cada ambiente + intervalo de confiança
memoriaEscritaExOutliers <- ggplot(tabelaEscritaExOutliers, aes(fill = Ambiente, x = GB, y = Tempo_Medio, group = Ambiente)) +
  geom_bar(position = 'dodge', stat = 'identity') +
  geom_errorbar(aes(ymin=Tempo_Medio - Intervalo_Confianca, ymax=Tempo_Medio+Intervalo_Confianca),
                position='dodge') +
  geom_errorbar(aes(ymin=Tempo_Medio - Intervalo_Confianca, ymax=Tempo_Medio+Intervalo_Confianca),
...
### Gráfico com Coeficiente de Variação dos ambientes 
### Esse gráfico tem o objetivo de apresentar a variação dos dados obtidos em porcentagem
coeficienteEscritaExOutliers <- ggplot(tabelaEscritaExOutliers, aes(fill = Ambiente, x = GB, y = Coeficiente_Variacao, group = Ambiente)) +
  geom_bar(position = 'dodge', stat = 'identity') +
  labs(title="Coefiente de Variação dos Resultados",x="GB", y = "Coeficiente_Variação (%)")+
  theme_bw(base_size = 20) +
  scale_fill_manual(values=c("#756bb1", "#fdae6b", "#31a354")) +
