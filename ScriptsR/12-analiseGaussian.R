  #####
  # Script R para analise de desempenho do Gaussian
  ...
  tabelaDados <- read.table('../Tabelas/GaussianHTTotal.log',
                              header = T)
  ...
  ggsave(filename = "../Graficos/Gaussian/01GaussianHT.png", width = 10, height = 7, dpi = 300) 
  ...
  ggsave(filename = "../Graficos/Gaussian/03GaussianCoeficienteHT.png", width = 10, height = 7, dpi = 300)
  ...
  tabelaComparada <- read.table('../Tabelas/GaussianHTTotal.log',
                                header = T)
  ...
  progBoxHT <- ggplot(tabelaComparada, aes(x=CPUs, y=Tempo, fill=Ambiente)) + 
    ...
    scale_fill_grey() +
    theme(legend.position = c(0.9, 0.8))
  ggsave(filename = "../Graficos/Gaussian/02GaussianBoxHT.png", width = 10, height = 7, dpi = 300)
  ...
  ### Análise de desempenho do Gaussian sem HyperThreading
  ### Esta análise considera apenas os 3 ambientes
  tabelaDados2 <- read.table('../Tabelas/GaussiansemHTTotal.log',
                            header = T)
  ...
  progsemHT <- ggplot(tabelaDados2, aes(fill = Ambiente, x = CPUs, y = Tempo_Medio, group = Ambiente)) +
    ...
    scale_fill_grey() +
    theme(legend.position = c(0.9, 0.8))
  ggsave(filename = "../Graficos/Gaussian/04GaussiansemHT.png", width = 10, height = 7, dpi = 300) 
  ...
  progCoeficienteSemHT <- ggplot(tabelaDados2, aes(fill = Ambiente, x = CPUs, y = Coeficiente_Variacao, group = Ambiente)) +
    ...
    scale_fill_grey() +
    theme(legend.position = c(0.2, 0.8))
  ggsave(filename = "../Graficos/Gaussian/06GaussianCoeficientesemHT.png", width = 10, height = 7, dpi = 300)
  
  ### BoxPlot
  ### Este gráfico tem o objetivo de verificar a variação dos dados obtidos por meio de quartis 
  ### ele indica a mediana e a variabilidade fora do quartil superior, inferior e outliers
  tabelaComparada2 <- read.table('../Tabelas/GaussiansemHTTotal.log',
                                header = T)
  ...
  progBoxsemHT <- ggplot(tabelaComparada2, aes(x=CPUs, y=Tempo, fill=Ambiente)) + 
    ...
    scale_fill_grey() +
    theme(legend.position = c(0.9, 0.8))
  ggsave(filename = "../Graficos/Gaussian/05GaussianBoxsemHT.png", width = 10, height = 7, dpi = 300)
