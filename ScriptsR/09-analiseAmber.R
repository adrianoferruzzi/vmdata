  #####
  # Script R para analise de desempenho do Amber
  ...
  ### Análise de desempenho do Amber com HyperThreading
  ### Esta análise considera apenas os 3 ambientes
  tabelaDados <- read.table('../Tabelas/AmberHTTotal.log',
                              header = T)
  ...
  ggsave(filename = "../Graficos/Amber/01AmberHT.png", width = 10, height = 7, dpi = 300) 
  ...
  ggsave(filename = "../Graficos/Amber/03amberCoeficienteHT.png", width = 10, height = 7, dpi = 300)
  ...
  tabelaComparada <- read.table('../Tabelas/AmberHTTotal.log',
                                header = T)
  ...
  progBoxHT <- ggplot(tabelaComparada, aes(x=CPUs, y=Tempo, fill=Ambiente)) + 
    ...
    scale_fill_grey() +
    theme(legend.position = c(0.9, 0.8))
  ggsave(filename = "../Graficos/Amber/02amberBoxHT.png", width = 10, height = 7, dpi = 300)
  ...
  ### Análise de desempenho do Amber sem HyperThreading
  ### Esta análise considera apenas os 3 ambientes
  tabelaDados2 <- read.table('../Tabelas/AmbersemHTTotal.log',
                            header = T)
  ...
  progsemHT <- ggplot(tabelaDados2, aes(fill = Ambiente, x = CPUs, y = Tempo_Medio, group = Ambiente)) +
    ...
    scale_fill_grey() +
    theme(legend.position = c(0.9, 0.8))
  ggsave(filename = "../Graficos/Amber/04AmbersemHT.png", width = 10, height = 7, dpi = 300) 
  ...
  progCoeficienteSemHT <- ggplot(tabelaDados2, aes(fill = Ambiente, x = CPUs, y = Coeficiente_Variacao, group = Ambiente)) +
    ...
    scale_fill_grey() +
    theme(legend.position = c(0.2, 0.8))
  ggsave(filename = "../Graficos/Amber/06amberCoeficientesemHT.png", width = 10, height = 7, dpi = 300)
  
  ### BoxPlot
  ### Este gráfico tem o objetivo de verificar a variação dos dados obtidos por meio de quartis 
  ### ele indica a mediana e a variabilidade fora do quartil superior, inferior e outliers
  tabelaComparada2 <- read.table('../Tabelas/AmbersemHTTotal.log',
                                header = T)
  ...
  progBoxsemHT <- ggplot(tabelaComparada2, aes(x=CPUs, y=Tempo, fill=Ambiente)) + 
    ...
    scale_fill_grey() +
    theme(legend.position = c(0.9, 0.8))
  ggsave(filename = "../Graficos/Amber/05amberBoxsemHT.png", width = 10, height = 7, dpi = 300)
